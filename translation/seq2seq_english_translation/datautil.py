...
# 获取文件文本
def get_ch_path_text(data_english, Isch=True, normalize_digits=False):
    text_files, _ = getRawFileList(data_english)
    labels = []
...
def fenci(training_data):
    seg_list = jieba.cut(training_data, cut_all=False, HMM=True)  # 默认是精确模式
...
def create_vocabulary(vocabulary_file, data_english, max_vocabulary_size, Isch=True, normalize_digits=True):
    texts, textssz = get_ch_path_text(data_english, Isch, normalize_digits)
...
data_dir = '/home/zutnlp/pycharm/zutnlp_english_translation/'
data_english = '/home/zutnlp/pycharm/zutnlp_english_translation/data/from'
data_chinese = '/home/zutnlp/pycharm/zutnlp_english_translation/data/to'
vocabulary_fileen = 'dicten.txt'
vocabulary_filech = 'dictch.txt'
...
def main():
    ...
    textdir_to_idsdir(data_english, data_dir + "fromids/", vocaben, normalize_digits=True, Isch=False)
    textdir_to_idsdir(data_chinese, data_dir + "toids/", vocabch, normalize_digits=True, Isch=True)
