...
class LargeConfigChinese(object):
  ...
  vocab_size = 60000
  target_num = 44 # 7 NER Tags: nt, n, p, o, q (special), nz(entity_name), nbz(brand)
  bi_direction = True
  crf_layer = True
  ...
  for step, (x, y) in enumerate(reader.iterator(word_data, tag_data, model.batch_size, model.num_steps)):
    ...
    if model.is_training:
      if step % (epoch_size // 10) == 10:
        ...
        # model.saver.save(session, 'ner.ckpt',checkpoint_path)
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    if ckpt:
      ...
      # print("Loading model parameters from %s" %' /deepnlp/deepnlp/ner/ckpt/zh')
